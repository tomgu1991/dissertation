...
在图\ref{fig:3-3-path-example}中，给出了图\ref{fig:1-1-example}代码片段的路径信息。
...
IMChecker的缺陷检测算法如\ref{alg:cp3-3-detect}中所示，以分解后的SpecMap和符号路径轨迹映射Trace\_Map作为输出，以缺陷检测结果R为输出。
...
路径分析包含三个部分，对应于规约的三个模式，即IPU、IEH和ICC。

SatisfyIPU对使用约束中的前置条件进行检测。
（1）如果在路径$t$中存在语义等价的\textbf{Assume}，
那么该约束$p$则满足；反之如果\textbf{Assume}与$p$的语义想法，那么该路径则一定错误。
（2）如果在路径$t$中不存在语义等价的\textbf{Assume}，那么则在路径的值映射关系$V$中判断$p$是否满足。
如果满足则为路径对目标接口使用正确，反之不正确。

SatisfyIEH对使用约束中后置条件中异常处理相关的约束进行检测。
每一个$eh=(cod, a^+)$由两部分组成，
错误路径检测条件$cod$（即，在$cod$满足的情况下是目标接口发生错误，
包含目标接口的返回值$ret$），
和异常处理动作$a^+$（即，异常处理路径上需要进行的异常处理操作）。
因此，在缺陷检测时，首先判断路径是否为缺陷检测路径，在判断异常处理动作是否满足约束。
...
其中调用约束对\textbf{Call}动作分析，返回值约束对\textbf{Return}动作分析。
...
（2）
$cod$不为True。
那么一方面，如果存在\textbf{Assume}动作等价于$cod$，则需要对$a$进行检查。
...
接口\texttt{X509\_get\_pubkey()}用于解码证书，当发生错误是，其返回NULL作为错误代码。
...
\end{lstlisting}
然而在图\ref{fig:3-3-path-example}中，只有路径$t_2$进行了显示的检测。
即，存在路径动作
\begin{lstlisting}[language={[ANSI]C},
basicstyle=\linespread{0.8}\listingsfont,
numbers=none,
...
\subsection{检测结果过滤}
为了支持规模化接口缺陷检测，IMChecker基于多入口分析策略，并在分析过程中对函数展开和循环展开进行限制。
这种分析策略会损失跨越函数的上下文信息，降低静态分析的精度，导致大量的误报。
...
$g$为是$f$因果调用函数，$H$为$c$中其他调用的函数。
则过滤的模式为：
\begin{itemize}
	\item $r$赋值给$c$的参数。一种常见的情况为，$f$为资源申请函数，并将申请后的指针$r$赋值给$c$的参数。
	...
	例如上述例子中$t_3$的情况。
	\item $g$在将$r$作为参数的$h \in H$中调用。即，$f-g$协同使用同时$r$需要满足约束。
	在$c$中，$r$满足约束，并作为$h$的参数，而且$g$在$h$中调用。
	例如下列代码，
			\begin{lstlisting}[language={[ANSI]C},
	basicstyle=\linespread{0.7}\listingsfont,
	numbers=none,
